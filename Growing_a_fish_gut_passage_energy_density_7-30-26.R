...
Ae                   <- numeric(n_days)
consumption_J_indv   <- numeric(n_days)
growth_J_indv       <- numeric(n_days)
energy_density_joules <- numeric(n_days)
Cmax                <- numeric(n_days)
consumption_J_day   <- numeric(n_days)
assimilated_J_day    <- numeric(n_days)
growth_J_day        <- numeric(n_days)
consumption_mg_day  <- numeric(n_days)
consumption_g_day  <- numeric(n_days)
...
for(i in seq_len(n_days)) {
  ...
  ########Consumption
  #C = RH/T, R = average reconstructed gut contents, H = hours of active feeding. T = gut passage time of active feeding larvae
  #https://onlinelibrary.wiley.com/doi/10.1111/j.1095-8649.1997.tb01533.x 
  
  #Consumption
  #https://cdnsciencepub.com/doi/10.1139/f97-176 (Stockwell and Johnson)
  #Type II - Consumption per minute = (SV*Prey_Density/(1+SV*Prey_Density*handlingtime))*60
  #Consumption per minute = (SV*Prey_Density/(1+SV*Prey_Density*handlingtime))*60
  #(SV[i]*density_today)/(1+V[i]*density_today*handlingtime)
  
  ## Consumption (µg/day)
  #consumption_ugPerday[i] <- captured[i] * Zoop_Weight_ug
  #convert to J/indv
  #Jindv = ug * J_per_ug / # of individuals 
  #  consumption_ugPerday[i] * J_per_ug/captured[i]
  ...
  #"It is possible to model egestion and excretion as functions of temperature and ration (e.g. Elliott,1976; Cui & Wootton, 1988) 
  ...
  #https://onlinelibrary.wiley.com/doi/full/10.1111/eff.12498 Pothoven 2019 
  Weight_g[i] <- 0.00000188 * Pred_Length[i]^3.296 #wet mass (I think)
  #Weight_g[i]
  ...
  #Assimilation Efficiency (already accounts for egestion)
  #Ae = 100(Growth+Rt)/C = AeC/100 - Rt(total metabolic rate) = Q(Growth)
  #Ae[i]*C[i]/100 - R[i] = Growth[i]
  #Ae = 43.079816 + 2.297390(Temp_C) -0.000001922(Mass_ug_of_fish) 
  #Ae[i] <- 43.079816 + 2.297390*(Temp_today) - 0.000001922*(Weight_g[i]) - U_excret[i]
  
  #Huuskonen assumes a constant assimilation efficiency of 75.3% of ingested energy (ignore more technical calculations)
  Ae[i] = 75.3
  
  #Coefficients from Huuskonen 1998
  CA = 0.07692 #Intercept for Cmax
  CB = -0.61966 #Coefficient Cmax v. Mass(wet; predator)
  CQ = 0.08007 #Coefficient e^c*T
  p = 1 #proportionality coefficient, picked arbitrary value based on ok feeding conditions
  
  Cmax[i] = CA * Weight_mg[i]^CB * exp(CQ*Temp_today) #relative consumption rate 
  #Cmax[i]
  consumption_mg_day[i] <- p * Cmax[i] * Weight_mg[i]
  #consumption_mg_day[i]
  consumption_g_day[i] <- consumption_mg_day[i] / 1000
  #consumption_g_day[i]
  
  #Energy Density of Daphnia (Daphnia Magna)
  #967.5 J/g wet weight (Wissing & Hasler, 1971; Hoefnagel et al. 2018)
  
  PreyED = 967.5 * 0.001 #J/mg
  
  consumption_J_day[i] = consumption_mg_day[i] * PreyED #energy in Joules 
  #consumption_J_day[i]
  
  assimilated_J_day[i] = consumption_J_day[i] * Ae[i]/100 
  #assimilated_J_day[i]
  ...
  #Respiration modified to be temperature dependent, #total metabolic rate 
  #mass of oxygen used per gram of an organism's body weight over a single day
  R[i] <-
    RA *
    Weight_mg[i]^RB *
    exp(RQ * Temp_today) * A
  ...
  respiration_J_day[i] <- R[i] * Weight_g[i] * Oxy_cal
  #respiration_J_day[i]
  
  growth_J_day[i] = assimilated_J_day[i] - respiration_J_day[i]
  #growth_J_day[i]
  
  energy_density_joules = 4286 #Joules per gram of Wet Mass
  
  growth_mg[i] = growth_J_day[i] / energy_density_joules
  #growth_mg[i]
  
  growth_g[i] = growth_mg[i]/1000
  #growth_g[i]
...
results <- data.frame(
  ...
  Growth_g = growth_g,
  Ae = Ae,
  consumption_J_indv = consumption_J_indv,
  growth_J_indv = growth_J_indv
...
###############
#doesn't make sense that days elasped is so much shorter in May than March, values are not biologically realistic
#what is causing this issue?

MonthlyDaphnia = GenevaZoops %>% group_by(month) %>%
  summarise(mean_zoop_value = mean(zoop_value))
MonthlyDaphnia
